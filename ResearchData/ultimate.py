...
baseFilesTofind = ["acc.py", "acc.s", "pythonperf", "0.serverperf", "1.serverperf"]

def makeHeader():
  header = ["Number of trees", "max depth", "max per cluster", "max unknown"]
  header.extend(["replicas", "dic splits", "table splits", "numSamples"])
  header.extend(["python exec time", "python acc", "server acc"])
  header.extend(["min core time", "max core time", "average core time in ms"])
  header.extend(["min diff in ms", "max diff in ms", "cores with no answer"])
...
def addTimes(l, row, times):
  ...
  minDiff = average - minimum
  maxDiff = maximum - average
  row[findSubstr(header, "min core time")] = minimum
  row[findSubstr(header, "max core time")] = maximum
  row[findSubstr(header, "average core time in ms")] = average
  row[findSubstr(header, "min diff in ms")] = minDiff
  row[findSubstr(header, "max diff in ms")] = maxDiff
...
def populateRow(segment, l, row, timesPerCore):
  global filesToFind
  if len(l) == 0:
    return
  ...
  elif "replica" in segment:
    return addTimes(l, row, timesPerCore)
  elif "0" in segment:
    return findPerf(l, row, "server m0")
  else:
    return findPerf(l, row, "server m1")

def expandFilesToFind(nameData):
    global filesToFind
    numReplicas = int(nameData[5])
    numDicSplits = int(nameData[6])
    numTabSplits = int(nameData[7])
    for i in range(numReplicas):
        for j in range(numDicSplits):
            for k in range(numTabSplits):
                extraFile = "replica" + str(i) + ".dicSplit" + str(j)
                extraFile = extraFile + ".tabSplit" + str(k) + ".time.txt"
                filesToFind.append(extraFile)
    return
...
with open(outfile, "w") as csvfile:
  ...
  for f in sorted(os.listdir(indir)):
    filesToFind = baseFilesTofind[:]
    row = ["_" for i in range(len(header))]
    timesPerCore = []
    nameData = f.split(".")
    row[0:8] = [int(x) for x in nameData[1:-1]]
    expandFilesToFind(nameData)
    temp = indir + f
    content = open(temp, "r").readlines()
    delims = findLims(content)
    for i in range(len(filesToFind)):
      try:
        populateRow(filesToFind[i], content[delims[i][0]:delims[i][1]], row, timesPerCore) 
      except:
        pass
    trimmedRow = [str(x).strip() for x in row]
    c.writerow(trimmedRow)
